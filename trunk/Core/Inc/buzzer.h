#ifndef __BUZZER_H__           // 이 파일이 중복 포함되는 것을 방지하기 위한 전처리기 지시문
#define __BUZZER_H__

#include "stm32f4xx_hal.h"    // HAL 라이브러리 사용을 위한 STM32F4 시리즈의 헤더 포함

// ?? 음계 주파수 정의 (Hz)
// 옥타브 C4 ~ B5의 음을 정의함. tone() 함수 호출 시 사용 가능
#define NOTE_B0  31
#define NOTE_C1  33
#define NOTE_CS1 35
#define NOTE_D1  37
#define NOTE_DS1 39
#define NOTE_E1  41
#define NOTE_F1  44
#define NOTE_FS1 46
#define NOTE_G1  49
#define NOTE_GS1 52
#define NOTE_A1  55
#define NOTE_AS1 58
#define NOTE_B1  62

#define NOTE_C2  65
#define NOTE_CS2 69
#define NOTE_D2  73
#define NOTE_DS2 78
#define NOTE_E2  82
#define NOTE_F2  87
#define NOTE_FS2 93
#define NOTE_G2  98
#define NOTE_GS2 104
#define NOTE_A2  110
#define NOTE_AS2 117
#define NOTE_B2  123

#define NOTE_C3  131
#define NOTE_CS3 139
#define NOTE_D3  147
#define NOTE_DS3 156
#define NOTE_E3  165
#define NOTE_F3  175
#define NOTE_FS3 185
#define NOTE_G3  196
#define NOTE_GS3 208
#define NOTE_A3  220
#define NOTE_AS3 233
#define NOTE_B3  247

#define NOTE_C4  262   // 도
#define NOTE_CS4 277
#define NOTE_D4  294   // 레
#define NOTE_DS4 311
#define NOTE_E4  330   // 미
#define NOTE_F4  349   // 파
#define NOTE_FS4 370
#define NOTE_G4  392   // 솔
#define NOTE_GS4 415
#define NOTE_A4  440   // 라
#define NOTE_AS4 466
#define NOTE_B4  494   // 시

#define NOTE_C5  523
#define NOTE_CS5 554
#define NOTE_D5  587
#define NOTE_DS5 622
#define NOTE_E5  659
#define NOTE_F5  698
#define NOTE_FS5 740
#define NOTE_G5  784
#define NOTE_GS5 831
#define NOTE_A5  880
#define NOTE_AS5 932
#define NOTE_B5  988

#define NOTE_C6  1047
#define NOTE_CS6 1109
#define NOTE_D6  1175
#define NOTE_DS6 1245
#define NOTE_E6  1319
#define NOTE_F6  1397
#define NOTE_FS6 1480
#define NOTE_G6  1568
#define NOTE_GS6 1661
#define NOTE_A6  1760
#define NOTE_AS6 1865
#define NOTE_B6  1976

typedef struct {
    TIM_HandleTypeDef* htim;     // 사용할 타이머 핸들
    uint32_t channel;            // PWM 채널 (예: TIM_CHANNEL_1)
    GPIO_TypeDef* gpio_port;     // 부저의 GPIO 포트 (예: GPIOA)
    uint16_t gpio_pin;           // 부저의 GPIO 핀 번호 (예: GPIO_PIN_15)
} Buzzer_HandleTypeDef;

void buzzer_init(Buzzer_HandleTypeDef* hbuzzer,
                 TIM_HandleTypeDef* htim,
                 uint32_t channel,
                 GPIO_TypeDef* gpio_port,
                 uint16_t gpio_pin);

//음계 및 멜로디
extern int tank_melody[];           // 재생할 음들의 주파수 배열 (예: {NOTE_C4, NOTE_D4, ...})
extern int tank_noteDurations[];    // 각 음의 지속 시간 (ms 단위)

extern int arcade_melody[];
extern int arcade_noteDurations[];

extern int main_melody[];
extern int main_noteDurations[];

void buzzer_tone(Buzzer_HandleTypeDef* hbuzzer, uint32_t freq, uint32_t duration_ms);
void buzzer_noTone(Buzzer_HandleTypeDef* hbuzzer);

// ?? noTone: 해당 타이머와 채널의 PWM을 멈춰 소리를 끔
void noTone(TIM_HandleTypeDef* htim, uint32_t channel);

//tank game bgm(shooting game)
void playTankBGM(Buzzer_HandleTypeDef* hbuzzer, uint8_t delay);
void playArcadeBGM(Buzzer_HandleTypeDef* hbuzzer, uint8_t delay);
void playMainTheme(Buzzer_HandleTypeDef* hbuzzer, uint8_t delay);

#endif  // __BUZZER_H__
